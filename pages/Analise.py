...
with hdr_l:
    st.title("📈 Análise de Membros (TRIBO)")
with hdr_r:
    # 🔁 Botão de mudança de modo (TRIBO -> ALDEIA)
    if st.button("🔁 Modo Aldeia", use_container_width=True, key="btn_modo_aldeia_hdr_analise"):
    ...
    if not mask_atrasados.any():
        ...
        g1 = (
            pd.DataFrame({"tsup": tsup_norm[mask_atrasados]})
              ...
              .sort_values("tsup")             # mantém ordem cronológica
        )
        g1["label"] = g1["tsup"].dt.strftime("%d/%m")

        # 👉 janela FIXA de 6 dias e scroller começando no INÍCIO
        n   = len(g1)
        ...
        options_g1 = {
            "backgroundColor": "transparent",
            "tooltip": {"trigger": "axis"},
            # menos margem à esquerda porque removemos o eixo Y
            ...
            # 👉 remove completamente o eixo Y (valores 0–1, ticks e linhas)
            "yAxis": {
                "type": "value",
...
with col_right:
    ...
    if not mask_atrasados.any():
        ...
        g2 = g2.sort_values("qtd", ascending=False).reset_index(drop=True)

        # 👉 scroller começa no início (mostrando os maiores já no topo)
        n   = len(g2)
